/* ARRAYS
  ...
  Los arrays son objetos y, como objetos, debemos crearlos con la instrucción new.
  Si damos valores iniciales en el momento de declarar un array:
    int[] a = {1,2,3};
  el compilador crea el objeto por nosotros.
  
  Los elementos que contiene un array son accedidos a través de su índice.
  El primer índice de un array es 0. El último, el tamaño del array menos 1.
  
  Los arrays tienen un atributo: length. Este atributo nos da el tamaño del array.
...
public class PruebaArrays {
    ...
    public static void main(String [] args) {
		...
		if (nombre.length != edad.length) {
			System.out.println("Error: los arrays deberÃ­an tener el mismo tamaÃ±o!!");
		}
		...
		// Imprimimos los datos leÃ­dos en orden inverso
		for(int i=4; i>=0; i--) {
			System.out.println("Nombre: " + nombre[i] + " Edad: " + edad[i] + " Salario: " + salario[i]);
		}
		*/
		// TambiÃ©n podemos almacenar en un array los objetos de nuestras propias clases
		Empleado[] empleados = new Empleado[5];   
		
		// Nota: observa cÃ³mo la orientación objetos nos permite agrupar datos, como los de un empleado,
		...
		*/
		...
		int[] frecuencias = new int[11]; // contadores de notas
		...
		// mostrar contadores
		for(int i=1;i<=10;i++) {
			System.out.println("Nota=" + i + " Veces: " + frecuencias[i]);
		}
		// calificación más frecuente: calcular máximo valor en array
		int max = frecuencias[1];
		for(int i=2; i<=10;i++) {
			if(frecuencias[i]>max) {
				max=frecuencias[i];
				// no almacenamos i (posición) porque podría haber
				// varios valores iguales en array
			}
		}
		System.out.println("Frecuencia máxima es: " + max + " en calificaciones:");
		for(int i=1;i<=10;i++) {
			if(frecuencias[i]==max) {
				System.out.print(i + " ");
			}
		...
		*/
		...
		System.out.println( nombreMes(14) );
		System.out.println( nombreMes(3) );
		
	}
	
	public static String nombreMes(int numeroMes) {
		String[] meses = {"ERROR", "enero", "febrero", "marzo"};
		String str;
		// ¿podemos ahorrarnos un switch-case o muchos if-else?
		// Pues sí...
...
/* 
   ...
   Ejercicio 3:
       AÃ±ade código para rellenar el array de empleados e imprimir luego los valores almacenados.
   ...
   Ejercicio 6:
       Codifica y prueba un método:
	      public static String nombreMes(int numeroMes) {}
	   que recibe un número de mes (entre 1 y 12) y devuelve su nombre o "Error" si el número está fuera
	   rango.
